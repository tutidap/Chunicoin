...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::C2C);
    ...
    if (!settings.contains("nAnonymizeChunicoinAmount"))
        settings.setValue("nAnonymizeChunicoinAmount", 1000);

    nAnonymizeChunicoinAmount = settings.value("nAnonymizeChunicoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeChunicoinAmount"))
        SoftSetArg("-anonymizechunicoinamount", settings.value("nAnonymizeChunicoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in chunicoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeChunicoinAmount:
            return QVariant(nAnonymizeChunicoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeChunicoinAmount:
            nAnonymizeChunicoinAmount = value.toInt();
            settings.setValue("nAnonymizeChunicoinAmount", nAnonymizeChunicoinAmount);
            emit anonymizeChunicoinAmountChanged(nAnonymizeChunicoinAmount);
            break;
        case CoinControlFeatures:
