...
#include "ui_zc2ccontroldialog.h"
...
std::list<std::string> ZC2CControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZC2CControlDialog::listMints;

ZC2CControlDialog::ZC2CControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZC2CControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZC2CControlDialog::~ZC2CControlDialog()
{
    delete ui;
}

void ZC2CControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZC2CControlDialog::updateList()
{
...
void ZC2CControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZC2CControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZC2C_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZC2CControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZC2CControlDialog::GetSelectedMints()
{
...
void ZC2CControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
