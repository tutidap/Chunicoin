                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zC2C ought to be enough for anybody." - Bill Gates, 2017
    ui->zC2CpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzC2CSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzC2C->setEnabled(false);
        ui->pushButtonMintzC2C->setToolTip(tr("zC2C is currently disabled due to maintenance."));

        ui->pushButtonSpendzC2C->setEnabled(false);
        ui->pushButtonSpendzC2C->setToolTip(tr("zC2C is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zC2CpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzC2C_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zC2C is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zC2C...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zC2C in ") +
...
void PrivacyDialog::on_pushButtonSpendzC2C_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zC2C is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zC2C
        sendzC2C();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zC2C
    sendzC2C();
}

void PrivacyDialog::on_pushButtonZC2CControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZC2CControlDialog* zC2CControl = new ZC2CControlDialog(this);
    zC2CControl->setModel(walletModel);
    zC2CControl->exec();
}

void PrivacyDialog::setZC2CControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzC2CSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzC2C()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Chunicoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zC2CpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zC2C is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " C2C </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zC2C</b>";
    ...
    // use mints from zC2C selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZC2CControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZC2CControlDialog::GetSelectedMints();
    }
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zC2C was spent successfully update the addressbook with the label
    ...
    // Clear zc2c selector in case it was used
    ZC2CControlDialog::listSelectedMints.clear();
    ui->labelzC2CSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zC2C Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " C2C, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zC2C Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zC2C </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zC2C "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zC2C "));
    ui->labelzC2CAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zC2C </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zC2C </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzC2CSyncStatus->setVisible(fShow);
}
